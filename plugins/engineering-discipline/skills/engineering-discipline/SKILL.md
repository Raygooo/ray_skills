...
description: |
  Set up and maintain a six-document source-of-truth system for a software project — strategic roadmap, immutable architecture decisions (ADRs), feedback and improvements backlog, pre-implementation rubric, knowledge-points learning log, and a cross-linked changelog — plus an optional agent-evolution layer on top: an autonomous evolution loop (cron / `/loop` / `/schedule`) and a parent-driven sub-agent prompt (Agent tool), both built on a shared implementation core. The system creates a forward and backward audit trail for every meaningful decision and a safe substrate for delegating implementation work to Claude agents while keeping the human in the merge decision.

  Trigger when the user says things like: "set up engineering discipline", "add an ADR system", "bootstrap project documentation", "I want to track decisions and feedback like a senior engineer", "create a roadmap and decision log", "add a feature checklist before we ship", "help me document this project so future me can pick it up", "set up source-of-truth docs", "I'm working with AI and don't want to lose track of why we decided X". Also trigger when the user asks how to organize technical decisions, deferred work, or learnings in a way that scales beyond a single contributor or session.

  Also trigger when the user wants to add an autonomous-agent layer to a project that already has the six-doc substrate: "set up an autonomous evolution loop", "have agents work on improvements while I'm away", "fire a cron job that picks IMPs and implements them", "add /loop or /schedule for this project", "spawn sub-agents to implement IMPs", "delegate one IMP to a sub-agent with the rubric", "give me an Agent-tool prompt for IMP-NNN", "set up the wrapper-core agent prompts", "add the initialization gate for autonomous runs". The agent-evolution layer requires the six-doc substrate; if the user asks for it on a project without the substrate, set up the substrate first.

  Has three modes:
  - BOOTSTRAP — scaffold the six docs in an empty or existing project from templates; optionally also scaffold the agent-evolution layer (autorun core + autonomous wrapper + parent-driven sub-agent wrapper + cron script + 3 ADRs)
  - AUDIT — review an existing project and identify which parts of the system are missing (six-doc substrate AND the agent-evolution layer if relevant)
  - ONGOING-USE — advise on when to write each kind of entry, how to cite items by ID, ADR immutability rules, KP proposal triggers, how to fire the autonomous loop, how to spawn parent-driven sub-agents

  Skip this skill for: small one-off scripts, throwaway prototypes, projects with fewer than two contributors and shorter than a one-week lifespan. The discipline pays for itself only above a certain size threshold; the agent-evolution layer pays for itself only above an even higher threshold (the human must be willing to actually review autonomous run reports — otherwise the layer accumulates unreviewed branches and is worse than working manually).
---
